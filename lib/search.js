...
function groupedQuery(filters, extra, limit, offset) {
  ...
  const aggregates =
    "count(*) as total,sum(case when violationstatus='Open' then 1 else 0 end) as open,sum(case when class='C' then 1 else 0 end) as classc,sum(case when violationstatus='Open' and class='C' then 1 else 0 end) as openclassc,max(inspectiondate) as latest";
  const query = {
...
function detailsQuery(buildingId) {
  const value = String(buildingId || "").trim();
  if (!/^\d{1,10}$/.test(value))
    throw new Error("Invalid building identifier.");
  return {
    $select:
      "violationid,novdescription,class,violationstatus,novissueddate,apartment,story,inspectiondate,currentstatusdate,rentimpairing",
    $where: `buildingid='${value}'`,
    $order: "violationstatus DESC,novissueddate DESC,violationid DESC",
    $limit: "100",
  };
}
module.exports = {
