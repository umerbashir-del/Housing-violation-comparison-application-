...
function groupedQuery(filters, extra, limit, offset) {
  const fields =
    "buildingid,max(housenumber) as housenumber,max(streetname) as streetname,max(boro) as boro,max(zip) as zip,max(bin) as bin,max(bbl) as bbl,max(latitude) as latitude,max(longitude) as longitude";
  const aggregates =
